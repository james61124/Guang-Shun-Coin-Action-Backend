...
func getMyProductQuery(pr ProductRequest, userID string) (string, string, []interface{}) {
    // Base query without the ORDER BY clause
    baseQuery := `
        SELECT 
            p.productId, 
        ...
        LEFT JOIN 
            History h ON p.productId = h.productId
        INNER JOIN
            TrackingList t ON t.productId = p.productId AND t.userId = ?
        WHERE 
            p.category = ?
        GROUP BY 
            p.productId, p.productName, p.category, p.price, p.minBidPrice, p.startAt, p.endedAt
    ...
    // Prepare arguments
    var args []interface{}
    args = append(args, userID, pr.Category)
...
// return status, list of products
func myProduct(pr ProductRequest, UUID string) ([]response.ProductResponse, int, error) {
	var err error

	query, countQuery, args := getMyProductQuery(pr, UUID)

	// Execute the count query to get the total number of products matching the criteria
    var totalProductCount int
	var totalPages int
    err = mariadb.DB.QueryRow(countQuery, args[0], args[1], args[2], args[3], args[4]).Scan(&totalProductCount)
    if err != nil {
        logger.Error("[SHOP] " + err.Error())
        return nil, totalProductCount, err
    }
